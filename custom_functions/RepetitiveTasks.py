...
def cleanlab_label_errors(classifier, X, y):
    ...
    ranked_label_issues = find_label_issues(labels=y, pred_probs=pred_probs, return_indices_ranked_by="self_confidence")
...
def dataset_error(X, y, X_train, X_test, y_train, y_test):
    
    # Finding label errors in the dataset according to classifier using cleanlabs
    indexes = cleanlab_label_errors(CountBasedClassifier(),X,y)
    ...
    res = classification_evaluation(classifiers, X_train, X_test1, y_train, y_test1)

    return res


## remove entire error labels. and train and test data again...
